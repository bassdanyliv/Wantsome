...
const distances = [
  ...
  distances.map(city => city.distance * 0.621371).forEach(city => console.log(city + ' miles'));
  distances.filter(city => city.distance < 10000).forEach(city => console.log(city));
  let totalDistance = distances.map(city => city.distance).reduce((a, b) => a + b);
...
const releases = [
 ...
 //const result = releases.map(({id,title}) => [id, title]);
 //console.log(result)
...
newReleases.filter(v => v.rating === 5.0).forEach(v => console.log(v.id));
...
movieLists.map(v => v.videos.map(v => v.id)).forEach(v => console.log(v));
...
movieLists2.map(v => v.videos
        .forEach(v => console.log(v.id + ' ' + v.title + ' ' + v.boxarts
